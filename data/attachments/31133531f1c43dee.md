...
- Name: Contact/contact.spec.js >> Contact Service - DemoBlaze >> CONT_013 Verify multiple data entries Sathya
- Location: tests/Contact/contact.spec.js:146:13
...
Call log:
  - waiting for locator('#exampleModal') to be visible
    62 × locator resolved to hidden <div tabindex="-1" role="dialog" id="exampleModal" class="modal fade" aria-hidden="true" aria-labelledby="exampleModalLabel">…</div>
...
- generic [ref=e1]:
  ...
  - navigation [ref=e2]:
    ...
    - list [ref=e6]:
      ...
      - listitem [ref=e10]:
        - link "Contact" [active] [ref=e11] [cursor=pointer]:
          - /url: "#"
      - listitem [ref=e12]:
...
```ts
  ...
  5  |         this.page = page;
  6  | 
  7  |         this.contactLink = page.locator('a', {
  8  |             hasText: 'Contact'
  9  |         });
  10 | 
  11 |         this.contactModal = page.locator('#exampleModal');
  12 | 
  13 |         this.modalTitle = page.locator('#exampleModalLabel');
  14 | 
  15 |         this.emailInput = page.locator('#recipient-email');
  16 | 
  17 |         this.nameInput = page.locator('#recipient-name');
  18 | 
  19 |         this.messageInput = page.locator('#message-text');
  20 | 
  21 |         this.sendMessageBtn = page.locator('button', {
  22 |             hasText: 'Send message'
  23 |         });
  24 | 
  25 |         this.closeBtn = page.locator('button', {
  26 |             hasText: 'Close'
  27 |         });
  28 |     }
  29 | 
  30 |     async goto() {
  31 | 
  32 |         await this.page.goto('https://www.demoblaze.com/');
  33 |     }
  34 | 
  35 |     async openContactModal() {
  36 | 
  37 |         await this.contactLink.click();
  38 | 
> 39 |         await this.contactModal.waitFor({
     |                                 ^ Error: locator.waitFor: Test timeout of 30000ms exceeded.
  40 |             state: 'visible'
  41 |         });
  42 |     }
  43 | 
  44 |     async fillContactForm(email, name, message) {
  45 | 
  46 |         await this.emailInput.fill(email);
  47 | 
  48 |         await this.nameInput.fill(name);
  49 | 
  50 |         await this.messageInput.fill(message);
  51 |     }
  52 | 
  53 |     async sendMessage() {
  54 | 
  55 |         await this.sendMessageBtn.click();
  56 |     }
  57 | 
  58 |     async closeModal() {
  59 | 
  60 |         await this.closeBtn.click();
  61 |     }
  62 | }
```